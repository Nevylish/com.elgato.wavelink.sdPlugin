...
class WaveLinkClient extends AppClient {
    ...
    init(system, sdDevices) {
		...
		this.on(kJSONPropertyLevelmeterValuesChanged, [ 'MixerList' /*API 6*/, kJSONKeyMixerList, kJSONKeyIdentifier, kJSONKeyLocalMixer, kJSONKeyStreamMixer, kJSONKeyMicrophoneLevel], (inputOldList, inputList, outputIdentifier, outputLocal, outputStream, microphoneLevel) => {
			...
			if (this.isAPIVersionOlderAs(7)) {
                ...
                if (micConfig && hasMicrophoneChanged)
                {
	...
	setSelectedOutput(identifier, mixerID) {
        ...
        if (output != undefined) {
            ...
            if (output.identifier == 'PCM_IN_01_V_00_SD2') {
                name = this.localization['outputSelection']['streamOut'];
            } else {
                name = outputs.name;
            }
	...
	getMicrophone(identifier) {
        ...
        if (this.isAPIVersionOlderAs(7)) {
            ...
            const groupInput = this.inputs.find(groupInput => groupInput.identifier == 'Wave Link Mic In 1');

            if (groupInput?.isWaveMicInput)
                microphoneConfig = this.microphones.find(microphone => microphone.identifier == groupInput.inputs[0].identifier);
        }
